# Port Plan: OpenBuddy → DFRobot K10 (xiaozhi base)

This is the plan for the answers you gave: **start fresh**, firmware is a **xiaozhi-esp32
fork on the K10**, and you wanted a **backend recommendation**.
...
already have a more capable, more widely supported base — **xiaozhi-esp32 on the K10** —
...
| OpenBuddy piece | Verdict for K10 | Why |
|---|---|---|
...
| Pet states driven by Claude lifecycle hooks (Stop/PreToolUse/PostToolUse) | ✅ **Keep** | Maps cleanly to xiaozhi's `llm` emotion message → K10 screen |
...
| OpenBuddy firmware (ESP-IDF, LVGL, ES8311) | ❌ **Drop** | K10 ≠ M5Stack; xiaozhi-esp32 already handles K10's display/mic/speaker |
...
1. **Run the skeleton** with fakes; connect your K10 (point its WS URL at this server) and
   confirm the `hello` handshake + an echo round-trip. → `firmware/K10_BOARD_NOTES.md`
...
4. **Pet states**: map agent lifecycle hooks → xiaozhi `llm` emotion messages → K10 screen.
5. **(Optional)** ElevenLabs providers, then the React dashboard.

## Open questions to resolve as you build

- **K10 board target in your fork:** confirm your `BOARD_TYPE` and that uplink is Opus
...
- **Local vs. cloud host:** the server can run on the same LAN as the K10 (low latency, mDNS
